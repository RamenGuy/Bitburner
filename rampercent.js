...
export async function main(ns) {
	...
	const percent = Math.floor(ns.getServerUsedRam(server) / ns.getServerMaxRam(server) * 100);
	var bar = "";
	...
	ns.tprint('['+bar+'] ('+ns.getServerUsedRam(server)+'GB/'+ns.getServerMaxRam(server)+'GB, '+percent+'%)');
